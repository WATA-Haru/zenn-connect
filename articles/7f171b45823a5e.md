...
## Templaterの設定
`templater`ディレクトリ配下にTemplaterのテンプレートファイルを作成します。
...
なお、YAML front matterはPropertiesで表示され、以下のような見た目になります。
![uuid-note-and-assets ノート画面](/images/7f171b45823a5e/3-yaml-frontmatter.png)

以上で、テンプレートの作成は完了です。

## Hotkeys
Hotkeysを使って、特定のキーを押したときに`uuid-note-and-assets.md`で定義したテンプレート通りのメモが作成されるようにします。
...
[obsidian-front-matter-title](https://github.com/snezhig/obsidian-front-matter-title)を導入して、YAML front matterの`title`をファイルツリーに表示するようにします。
