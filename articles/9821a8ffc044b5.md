...
※以下では基本的に`NVIM_APPNAME`という言葉を概念を表す言葉として使いますが、環境変数そのものを指す場合は`$NVIM_APPNAME`と表記します。

## `NVIM_APPNAME`とはなにか
`$NVIM_APPNAME`はNeovimが使う設定用ディレクトリを制御する環境変数です。普段はUnix系では`~/.config/nvim`、Windowsでは`~/AppData/Local/nvim`から設定を読み込みますが、`$NVIM_APPNAME`の値を変えることで設定の読込先を変更できます。
...
例えば、Nvimがスタートする前に`$NVIM_APPNAME`として`foo`を設定用ディレクトリにしていたら、Nvimは`$XDG_CONFIG_HOME/nvim`の代わりに`$XDG_CONFIG_HOME/foo`を参照するようになります。
`$NVIM_APPNAME`は必ず"foo"のような名前か"foo/bar"のような相対パスでなければなりません。
...
`$XDG_CONFIG_HOME`(Unix系:`~/.config` / Windows:`~/AppData/Local`)配下にNeovimの設定用ディレクトリを作成します。
私はUbuntuを使っているので以下`~/.config`を`$XDG_CONFIG_HOME`として進めます。
...
直接Neovimの機能には関係しませんが、`$NVIM_APPNAME`をaliasと紐づけるとすぐ開けるようになって便利なのでお好みで設定してください。

参考までに、私は[fish](https://fishshell.com/)に`aliasを設定しています。
以下のコマンドでは`nn`を押すと`nvim-with-denops`が設定用ディレクトリとして読み込まれてNeovimが立ち上がります。

```config.fish
alias nn='NVIM_APPNAME=nvim-with-denops nvim'
```
これでNeovimのサブ環境の作成とaliasの設定が完了です。
