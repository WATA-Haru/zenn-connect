...
前述の通りUnix系では`~/.config`、/ Windows:`~/AppData/Local`が`$XDG_CONFIG_HOME`になります。私はUbuntuを使っているので以下`~/.config`を`$XDG_CONFIG_HOME`として進めます。
...
直接Neovimの機能には関係しませんが、`$NVIM_APPNAME`をalias`と紐づけるとすぐ開けるようになって便利なのでお好みで設定してください。
